...
2. 과학적 원리 또는 기술의 작동 원리를 제시할 때에는 원리를 피상적/광범위하게 나열하기 말고, 미시적이고 깊이 있게 설명하여 정보를 제공해 주세요.
3. 작가 모델이 다소 광범위한 주제에 대해 질문한다면, 작가 모델이 지문에 포함할 내용을 탐색하고 있는 것입니다. 다음의 서사 구조 중 하나를 살려 입체적으로 정보를 제공해 주세요.
...
4. 수능은 배경지식이 아닌 논리적 규칙을 이해하고 적용하는 시험입니다. 개념의 양과 다양성보다는 조건, 인과, 대립, 분기, 위계 등 논리적 규칙을 명확히 하여 정보를 제공해 주세요.
5. 수식을 통한 설명보다는, 언어를 활용하여 논리적으로 설명해 주세요.
"""
...
top_p = st.sidebar.slider("Top P", 0.0, 1.0, 0.85, 0.05)
presence_penalty = st.sidebar.slider("Presence Penalty", -1.0, 1.0, 0.05, 0.05)
